...
export default function usePermissions(
	...
	const createAllowed = computed(() => {
		...
		const hasJunctionPermissions = !!permissionsStore.permissions.find(
			(permission: any) => permission.action === 'create' && permission.collection === junctionCollection.value.collection
		);

		const hasRelatedPermissions = !!permissionsStore.permissions.find(
			(permission: any) => permission.action === 'create' && permission.collection === relationCollection.value.collection
		);
