<template>
  ...
  >
    ...
    >
      <template #subtitle>
        <v-breadcrumb
          :items="[{ name: collectionInfo.name, disabled: true }]"
        />
      </template>
...
import usePreset from "@directus-extensions/shared/composables/use-preset";
import { useCollection } from "@directus/shared/composables";
import { useLayout } from "@directus-extensions/shared/composables/use-layout";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const {
      ...
      search,
      filter: presetFilter,
    } = usePreset(collection, ref(null), true, presetsStore, usersStore);
    ...
    const localLayout = ref(layout.value || "tabular-img");
    ...
    const { layoutWrapper } = useLayout();

	// logger.log('layout wrapper')
    ...
    function useActions() {
      ...
      function save() {
        ...
        // logger.info(internalSelection.value)
