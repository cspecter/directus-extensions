...
import { addTokenToURL } from '../../utils/add-token-url';
...
export default defineComponent({
	...
	setup(props, { emit }) {
		const api = useApi();
		const { useFieldsStore, useRelationsStore, useUserStore, usePermissionsStore, useNotificationsStore } = useStores();
		...
		const { templateData, loading: templateDataLoading } = useTemplateData(templateCollection, templatePrimaryKey, api);
		...
		function useFile() {
			...
			const file = computed(() => {
				...
				const src = addTokenToURL(api, getRootPath() + `assets/${fileData.id}?key=system-large-contain`);
				return { ...fileData, src };
			});
