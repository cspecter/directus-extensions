<template>
	<div ref="templateEl" class="render-template-image">
		...
		<template v-for="(part, index) in parts" :key="index">
			...
			:src="part" 
			:alt="part" 
			v-else-if="typeof part === 'string' && part.search('https') > -1"
			/>
			<!-- <component
	...
	setup(props) {
		...
		const parts = computed(() =>
			props.template
				...
				.map((p) => p ?? null)
				.map((j) => {
					// if (pre && responseData.length > 0) {
					// const f = fields.value
					// const v = f[0] as string
					// const prep = v.replace("{{", "").replace("}}", "");
					// const values = prep.split(".")
					// if (values.length === 2) {
					// 	const v1: string = values[0] as string
					// 	const v2: string = values[1] as string
					// 	responseData = responseData.map((item) => {
					// 		item[v1][v2] = `${pre}${item[v1][v2]}`
					// 		return item
					// 	})
					// }
					return `${props.prefix}${j}`
				})
		);
